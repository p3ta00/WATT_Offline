...
![Diagram of Active Directory Organization showing a pyramid structure. Top: Domain Policy for enterprise-wide policy and licensing. Middle: Group Policy Object managing application portfolio and settings. Bottom: Marketing and Sales documents within Active Directory OU. Left: Active Directory Server. Right: Application Library and File Share.](214CBLESIJ0C.png)[Source](https://dbaontap.com/2016/11/25/oem13c-roles-ad-groups/active-directory/)
...
![Diagram of Forest A and Forest B. Forest A: Parent Domain with three OUs, Child Domain with three OUs. Forest B: Domain with three OUs.](ez0mPNLMktV3.png)
...
![Diagram of Active Directory Forests & Domains. Left: inlanefreight.local with subdomains corp, dev, wh.corp, and admin.dev. Right: freightlogistics.local with subdomains corp, dev, and admin.dev.](lv5dmy1zYCpm.png)

Enable step-by-step solutions for all questions
![sparkles-icon-decoration](c0OHKoZ5mHTv)
...
\+ 1  Active Directory provides authentication and <\_\_\_\_> within a Windows domain environment.
...
![Diagram of Active Directory structure showing Distinguished Name (DN) and Relative Distinguished Name (RDN) for inlanefreight.local. Includes Users, Sales, and Managers OUs, leading to user BJones. Notes: DN must be unique in the directory, RDN must be unique in an OU.](EnxKOGNbG8eY.png)
...
Enable step-by-step solutions for all questions
![sparkles-icon-decoration](QViIEERLFO9F)
...
![Icons representing Active Directory Objects: Domain, Computer, OU, Groups, User, and Printers.](lFrjjJJcqo88.png)
...
Enable step-by-step solutions for all questions
![sparkles-icon-decoration](KcI4j8FTd6Rk)
...
\+ 1  <\_\_\_> are objects that are used to store similar objects for ease of administration. (Fill in the blank)
...
![Diagram of Inlanefreight Forest showing trust types: Parent-child, Cross-link, External, Tree-root, and Forest. Domains include inlanefreight, corp.inlanefreight, wh.corp.inlanefreight, dev.inlanefreight, freightlogistics, and shippinglanes.](t2QawMIHKPbR.png)
...
Enable step-by-step solutions for all questions
![sparkles-icon-decoration](jdcpdqcFFuBo)
...
![Kerberos authentication process: Client requests TGT from KDC, receives TGT, requests TGS, receives TGS, and accesses database server. Includes steps for user login and ticket encryption.](K5pHk1pWbhjm.png)
...
![Diagram of DNS request process: User requests inlanefreight.com, DNS server provides IP 134.209.24.248, user makes HTTP request to IP, server responds.](KuHcojTC2qq1.png)
...
![Diagram showing LDAP process: Client application requests user info from API Gateway, which queries Active Directory via LDAP, and returns user info.](Zdg2f98jv80R.png)
...
Enable step-by-step solutions for all questions
![sparkles-icon-decoration](l6VbAbuj7hji)
...
![NTLM protocol diagram: Client sends NTLM Negotiate, Challenge, and Authenticate messages to Server (DC). Server responds with Netlogon network and validation info.](mNn3iqHa163N.png)
...
Enable step-by-step solutions for all questions
![sparkles-icon-decoration](t4jKCLhcHY53)
...
\+ 0  NTLM uses three messages to authenticate; Negotiate, Challenge, and <\_\_>. What is the missing message? (fill in the blank)
...
![Active Directory Users and Computers interface showing organizational units and security groups under INLANEFREIGHT.LOCAL, including Employees, Security Groups, and Service Accounts.](25CUt68P731D.png)
...
Enable step-by-step solutions for all questions
![sparkles-icon-decoration](FWPhhoOUmXlw)
...
![Active Directory Users and Computers interface with 'New Object - Group' dialog open. Fields for Group name, Group scope, and Group type are visible under INLANEFREIGHT.LOCAL/Corp/Employees/HQ-NYC/IT.](AbEp8uaNVRHF.png)
...
![Diagram showing connections between email addresses and groups: DCORNER@INLANEFREIGHT.LOCAL is a member of HELP DESK@INLANEFREIGHT.LOCAL, which connects to HELP DESK LEVEL 1@INLANEFREIGHT.LOCAL and TIER 1 ADMINS@INLANEFREIGHT.LOCAL.](ucBFir7bykWf.png)
...
Enable step-by-step solutions for all questions
![sparkles-icon-decoration](no0MCn0J24nF)
...
Enable step-by-step solutions for all questions
![sparkles-icon-decoration](lQcVlUITmOkd)
...
![Diagram of CIA Triad: Confidentiality, Integrity, Availability. Confidentiality focuses on access control, Integrity ensures data authenticity, and Availability ensures resource accessibility.](QF3HPd2VKdLt.png)
...
![Group Policy Management Editor showing Default Domain Policy under Computer Configuration. Advanced Audit Policy Configuration settings are not configured.](ZZXx23lD3U7U.png)
...
![Active Directory Users and Computers interface showing security groups under INLANEFREIGHT.LOCAL. Groups include Access Control Assistance Operators, Account Operators, Administrators, and Backup Operators, with descriptions of their permissions.](tlUyyWT0CfGr.png)
...
Enable step-by-step solutions for all questions
![sparkles-icon-decoration](kiGaR5HeOovp)
...
\+ 0  Confidentiality, <\_\_\_>, and Availability are the pillars of the CIA Triad. What term is missing? (fill in the blank)
...
![Group Policy Management Editor showing Remote Session Environment settings under Remote Desktop Services. Settings include RemoteFX for Windows Server 2008 R2 and various display and graphics configurations.](SWZ8WhFCqu9H.png)
...
![Diagram of Group Policy processing order: Local Security Policy, Site Policy, Domain Policy, Parent OU Policy, Child OU Policy. Descriptions of Computer, User, and Group Policy settings application.](aTLhurQOvx3e.png)
...
![Group Policy Management interface showing Corp OU under INLANEFREIGHT.LOCAL. Linked Group Policy Objects: Disallow LM Hash, Block Removable Media, Disable Guest Account, all enabled.](OJ5XIAgDK0x0.png)
...
![Group Policy Management interface showing Service Accounts under INLANEFREIGHT.LOCAL. Linked Group Policy Objects: Logon Banner, Disallow LM Hash, Block Removable Media, Disable Guest Account, Default Domain Policy, all enabled.](vdQmcO5KLoEt.png)
...
![Group Policy Management interface showing Computers under INLANEFREIGHT.LOCAL. Linked Group Policy Objects: Default Domain Policy, Disable Forced Restarts, Disallow LM Hash, Block Removable Media, Disable Guest Account, all enabled.](c7NPRpcXDnzK.png)
...
![Group Policy Management interface showing Computers under INLANEFREIGHT.LOCAL. Linked Group Policy Objects: Logon Banner, Disable Forced Restarts, Disallow LM Hash, Block Removable Media, Disable Guest Account, Default Domain Policy, all enabled.](6TLnkOBAnkOb.png)
...
![Group Policy Management interface showing Computers under INLANEFREIGHT.LOCAL. Linked Group Policy Objects: Logon Banner and Disable Forced Restarts, both enabled.](7eERm1RPR9Ji.png)
...
![Group Policy settings for refresh interval on computers. Options to configure interval in minutes, with a range of 0 to 44640 for policy application and 0 to 1440 for random time offset.](zsZt7BeRQtBK.png)
...
![Network diagram showing DOMAIN USERS@INLANEFREIGHT.LOCAL as a member of AUTHENTICATED USERS@INLANEFREIGHT.LOCAL, which has connections to DISCONNECT IDLE RDP@INLANEFREIGHT.LOCAL with permissions: GenericWrite, WriteOwner, WriteDacl.](NJaaVOWLffof.png)
...
Enable step-by-step solutions for all questions
![sparkles-icon-decoration](99hKbZPdRXT6)
...
\+ 0  Computer settings for Group Policies are gathered and applied at a <\_\_\_> minute interval? (answer is a number, fill in the blank )
...
![Email from Bucky Barnes to Helpdesk requesting tasks: add new hires to AD, remove inactive users, unlock Adam Masters' account, create security group and OU, validate DNS records, and apply Group Policy.](Gxs1zqACZNCU.png)
...
![Service ticket for Adam Masters: Account locked out. Priority 2 - Medium, Impact/Urgency: Low/Medium. Notes mention repeated lockouts after incorrect password entries.](RoQuuet5IJ9W.png)
...
![Active Directory Users and Computers interface showing the creation of a new user under the Corp organizational unit. Context menu highlights 'New' and 'User' options.](2n0gxj4PKC62.png)

1\. Right click on "IT", Select "New" > "User".


![New User creation form for Andromeda Cepheus in INLANEFREIGHT.LOCAL/Corp/Employees/HQ-NYC/IT. Fields include first name, last name, user logon name 'acepheus'.](ZxKEF3w1oIPF.png)

2\. Add the users First and Last name, set the "User Logon Name:" as \`acepheus\` and then hit Next.


![New User password setup form for INLANEFREIGHT.LOCAL/Corp/Employees/HQ-NYC/IT. Password fields filled, 'User must change password at next logon' checked.](VtxprQz61Mr5.png)

3\. Set a password of \`NewP@ssw0rd123!\`and check the box for "User must change password at next login".


![New User creation summary for Andromeda Cepheus in INLANEFREIGHT.LOCAL/Corp/Employees/HQ-NYC/IT. User logon name: acepheus. User must change password at next logon.](SqqSQyjwcNUg.png)

4\. If all attributes look correct, select "Finish" in the last window


![Active Directory Users and Computers interface showing user Andromeda Cepheus in the IT organizational unit under INLANEFREIGHT.LOCAL/Corp/Employees/HQ-NYC.](qIraVXRJfbkq.png)
...
- Now supply the new user with a password of `NewP@ssw0rd123!`, confirm the password again, and check the box for " User must change password at next login", then hit next.
Select "Finish" in the last window if all attributes look correct.
...
![Active Directory Users and Computers interface showing the context menu for the Employees organizational unit. 'Find' option is highlighted.](hH8kPhCPdXMq.png)

1\. Right click on the "Employees OU" and select "find".


![Find Users, Contacts, and Groups interface showing search for 'Paul Valencia' in Employees. Result: Paul Valencia, User.](keXnxRTGlUpj.png)

2\. Type in the username you wish to search for, in this case "Paul Valencia" and hit "Find Now".


![Find Users, Contacts, and Groups interface showing search for 'Paul Valencia' in Employees. Context menu with 'Delete' option highlighted for Paul Valencia, User.](WsJj4AFDM3j2.png)

3\. Right click on the user and select delete.


![Confirmation dialog in Active Directory Domain Services asking to delete user 'Paul Valencia' with 'Yes' and 'No' options.](A6GpDbn6Q5L7.png)
...
![Active Directory Users and Computers interface showing context menu for user Adam M. 'Reset Password' option is highlighted.](85RRye84HGfS.png)
...
![Active Directory Users and Computers interface showing context menu for creating a new Organizational Unit under Employees. 'New' and 'Organizational Unit' options are highlighted.](G7RwZDYJEjoT.png)

1\. From within the IT OU, Right click and select "New" > "Organizational Unit"


![Active Directory Users and Computers interface showing creation of a new Organizational Unit named 'Security Analysts' under INLANEFREIGHT.LOCAL/Corp/Employees/HQ-NYC/IT. 'Protect container from accidental deletion' is checked.](GTtgJSFSK5Dt.png)
...
![Active Directory Users and Computers interface showing context menu for creating a new Group under Security Groups. 'New' and 'Group' options are highlighted.](9fTLHyETfkOo.png)

1\. Right click on our new OU \`Security Analysts\` and select "New > Group". A popup window should appear.


![Active Directory interface showing creation of a new group named 'Security Analysts' in INLANEFREIGHT.LOCAL/Corp/Employees/HQ-NYC/IT. Group scope set to 'Domain local' and type to 'Security'.](YQWivPqdp9RT.png)
...
![Active Directory interface showing context menu for user Andromeda Cepheus. 'Add to a group' option is highlighted.](ZCtIFRtqSqeu.png)

1\. Right click on the user and select "Add to a group"


![Select Groups dialog in Active Directory. 'Security' entered in object names field, 'Check Names' clicked. 'Security Analysts' group selected from matching names.](NUMxBwCdxMnl.png)

2\. Enter a full or partial group name in the search box and hit "Check Names".


![Select Groups dialog in Active Directory. 'Security Analysts' entered in object names field, 'OK' button highlighted.](0bpwWnoOp4HU.png)
...
![Active Directory Users and Computers interface showing context menu for Security Analysts Control GPO. 'Edit' option is highlighted.](gLZCUYjDTA6v.png)

1\. Right-click the GPO we wish to modify and select "Edit". This will bring up the Group Policy Configuration Editor window.


![Group Policy Management Editor showing User Configuration for Removable Storage Access. 'All Removable Storage classes: Deny all access' is highlighted and not configured](Qws7QAwxllDK.png)
...
![Group Policy Management Editor showing Removable Storage Access settings. 'All Removable Storage classes: Deny all access' is highlighted with 'Edit' option selected.](tTcHlqC20sS4.png)

3\. Right click the setting and select "Edit".


![Group Policy setting for 'All Removable Storage classes: Deny all access' is enabled. Options to apply or cancel changes are highlighted.](0AzeXY31zNw7.png)

4\. Check the radial button to enable the setting, hit "Apply" and then "OK".


![Group Policy Management Editor showing Removable Storage Access settings. 'All Removable Storage classes: Deny all access' is enabled.](2vZf6SlHcuRz.png)

5\. We can now see that our Policy setting is set to Enbled. Once we push policy to the domain, it will take effect.


![Group Policy Management Editor showing User Configuration for System settings. 'Prevent access to the command prompt' is highlighted and not configured.](upoqUbruiGFt.png)

6\. Next we are modifying the policy for Command Prompt access. Move to the System hive under User Configuration.


![Group Policy Management Editor showing User Configuration for System settings. 'Prevent access to the command prompt' is highlighted with 'Edit' option selected.](pOMrhc6PIchY.png)

7\. Right click and Edit the setting for "Prevent access to the command prompt".


![Group Policy setting for 'Prevent access to the command prompt' is disabled. Options to apply or cancel changes are highlighted.](ioKDpvXFDQrV.png)
...
![Group Policy Management Editor showing System settings. 'Prevent access to the command prompt' is disabled.](PCW4EOLcvqsS.png)
...
![Group Policy Management Editor showing Computer Configuration for Security Options. 'Interactive logon: Message text for users attempting to log on' is highlighted and not defined.](Y3OEhJFC9vfC.png)
...
![Security policy setting for 'Interactive logon: Message text for users attempting to log on' is defined. Warning message about restricted access is displayed. 'OK' button is highlighted.](zje2MTXFYwO7.png)
...
![Security policy setting for 'Interactive logon: Message title for users attempting to log on' is defined as 'Computer Access Policy'. 'OK' button is highlighted.](0tkguSoevCk2.png)
...
![Group Policy Management Editor showing Computer Configuration for Password Policy. Policies like Enforce password history, Maximum password age, and others are not defined.](Z6J3JjT8SO1i.png)
...
![Group Policy Management Editor showing 'Minimum password length' policy set to 10 characters. 'Define this policy setting' is checked. 'OK' and 'Apply' buttons are highlighted.](STWBVVHcdWj4.png)
...
![Group Policy setting for 'Password must meet complexity requirements' is defined and enabled. 'OK' and 'Apply' buttons are highlighted.](azpLlZjCD9E8.png)
...
![Group Policy Management Editor showing 'Enforce password history' policy set to remember 5 passwords. 'Define this policy setting' is checked. 'OK' and 'Apply' buttons are highlighted.](1RYbcHMQ1ECk.png)
...
![Group Policy Management Editor showing 'Minimum password age' properties. Suggested change: Minimum password age set to 7 days, Maximum password age suggested to 30 days. 'OK' and 'Apply' buttons are highlighted.](DuTOxTAJAhnN.png)
...
![Group Policy Management Editor showing Password Policy settings: Enforce password history (5 passwords remembered), Maximum password age (30 days), Minimum password age (7 days), Minimum password length (10 characters), Password must meet complexity requirements (Enabled), Store passwords using reversible encryption (Not Defined).](lTO70VO6UIlj.png)
...
![System information for Windows 10 Pro showing processor, RAM, system type, and computer name. 'Change settings' option is highlighted.](TYXUj6JkVJIq.png)

1\. From the control panel, open up system properties for the pc. Click on Change Settings in the Computer name section.


![System Properties window showing Computer Name tab. Fields for computer description, full computer name 'ACADEMY-IAD-TEMPJ', and workgroup 'WORKGROUP'. 'Change' button is highlighted.](pgHmNmVZSIqg.png)

2\. In this window, select the Change button beside "rename this computer or change its domain."


![Computer Name/Domain Changes window showing computer name 'ACADEMY-IAD-TEMPJ' and domain 'INLANEFREIGHT.LOCAL'. 'OK' button is highlighted.](xEZCgGjsfztq.png)

3\. Enter the domain you wish to join the host to. (INLANEFREIGHT.LOCAL) Hit OK.


![Windows Security dialog for Computer Name/Domain Changes. Domain: INLANEFREIGHT.LOCAL\htb-student_adm with password field. 'OK' button is highlighted.](MhjbCWKr2sVx.png)

4\. Enter the domain administrator credentials provided to join the host to the domain.


![Computer Name/Domain Changes confirmation dialog. Message: 'Welcome to the INLANEFREIGHT.LOCAL domain.' 'OK' button is highlighted.](bIFCoKDGneKV.png)
...
![Active Directory Users and Computers interface showing Computers under INLANEFREIGHT.LOCAL. Context menu for ACADEMY-1 with 'Move' option highlighted.](hNCt37EgrzjB.png)

1\. Looking in the Computers OU, select our newly joined host and right click it. Select the option to "Move"


![Active Directory Users and Computers interface showing the 'Move' dialog for ACADEMY-1. 'Computers' container is highlighted for selection.](iIkNpbWnOfMX.png)

2\. In the popup, drill down to the Security Analysts OU.


![Move object dialog in Active Directory. 'Corp' and 'Security Analysts' containers are highlighted for selection. 'OK' button is highlighted.](ddgVoVbTKWtG.png)

3\. Select the Security Analysts OU and hit OK.


![Active Directory Users and Computers interface showing ACADEMY-IAD-TEM computer under INLANEFREIGHT.LOCAL in the Computers container.](ylUPvyssuduJ.png)
