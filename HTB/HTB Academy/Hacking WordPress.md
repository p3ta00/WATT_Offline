...
![WordPress plugins list showing 'Mail Masta' with options to activate or delete, and 'Photo Gallery' with option to deactivate.](P0S4XBLYHhJX.png)

If we browse to the plugins directory, we can see that we still have access to the `Mail Masta` plugin.

![Directory listing of /wp-content/plugins/mail-masta showing folders and files including amazon_api, inc, lib, plugin-interface.php, and readme.txt.](1Zzb2iHbMns3.png)
...
![Website with shipping containers labeled 'Important Information' and contact details for Inlanefreight.](rf3bdaX2NqXz.png)
...
![sparkles-icon-decoration](5pbO5dhU7Srx)
...
![Text file showing user accounts with usernames, IDs, home directories, and shell paths.](wcC3txBktGDF.png)
...
![WordPress theme editor showing Transportex stylesheet with theme details and code.](hOd85UIlGuUv.png)

We can see that the active theme is `Transportex` so an unused theme such as `Twenty Seventeen` should be chosen instead.

#### Selecting Theme

![WordPress theme editor showing Twenty Seventeen stylesheet with theme details and code.](urb0VHct0SKu.png)
