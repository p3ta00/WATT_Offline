...
![Windows search for 'Event Viewer' showing options: Open, Run as administrator, Open file location, Pin to Start, Pin to taskbar.](nI23Kc4RpNZZ.png)

![Windows Logs showing Application, Security, Setup, System, and Forwarded Events with event counts and sizes.](Q5AS6nggHdxV.png)
...
![Event Viewer displaying DLLHijack logs with details of Sysmon events, including registry value changes and process information.](EZe0Zru3zklS.png)
...
![Icons for Information and Error with counts.](av4uPuOj00Fq.png)
...
![Event Viewer error for SideBySide, Event ID 35, detailing activation context generation failure for Visual Studio with processor architecture mismatch.](ytYgTMQXygYc.png)
...
![Event Viewer details for SideBySide, Event ID 35, showing provider, version, level, and creation time.](ZHPYYwzMiibs.png)
...
![Event Viewer details showing SystemTime, EventRecordID 1773, ProcessID 636, ThreadID 0, on computer ARASHPARSA2BB9.](gWtRFby70kbI.png)
...
![Event Viewer log for Event 4624, Microsoft Windows security auditing, showing successful account logon with Security ID SYSTEM, Account Name ARASHPARSA2BB9$, and Logon Type 5.](VuI3rHpaAMYX.png)
...
![Event Viewer filter setup with XML query for Security log, filtering by SubjectLogonId 0x3E7.](FI4nmOwU6wgb.png)
...
![Event 4624 details: SubjectUserName ARASHPARSA2BB9$, SubjectDomainName WORKGROUP, TargetUserName SYSTEM, LogonType 5, LogonProcessName Advapi.](x8VS0dFM93k1.png)
...
![Event Viewer showing Event 4907, Microsoft Windows security auditing, with audit policy change details for account ARASHPARSA2BB9$, object path, and logon ID 0x3E7.](UWwf80GwvJhL.png)
...
![Event 4907 details: SubjectUserName ARASHPARSA2BB9$, SubjectDomainName WORKGROUP, ObjectType File, ObjectName path, ProcessName SetupHost.exe.](aIDSRLySweyz.png)
...
![Event IDs 4624 Logon and 4672 Special Logon.](9OULoeitnFiv.png)

Analyzing the special logon event, we gain insights into token permissions granted to the user upon a successful logon.

![Event 4672, Microsoft Windows security auditing, showing SYSTEM account with special privileges like SeAssignPrimaryTokenPrivilege and SeDebugPrivilege.](a7CO6TiBCmdk.png)
...
![Command prompt showing Sysmon v13.33 installation and startup messages.](AONKc11Cmjg4.png)
...
![XML snippet showing RuleGroup with ImageLoad set to 'include' and a note about no rules meaning nothing will be logged.](Pg0Z22g8jFqM.png)
...
![XML snippet with RuleGroup, ImageLoad set to 'exclude', and a note about using 'include' with no rules.](XUPx48Zxg6kR.png)
...
![Command prompt showing Sysmon v13.33 loading sysmonconfig-export.xml, configuration validated and updated.](u5RN30h9IVxi.png)
...
![Sysmon event log showing multiple Information entries for Event ID 7, Image loaded.](j4LvHVH0GX2G.png)

Let's now see how a Sysmon event ID 7 looks like.

![Sysmon log entry: Image loaded, ProcessID 8060, Image mmc.exe, ImageLoaded psapi.dll, Signed true, User DESKTOP-N33HELB\Waldo.](1MR8o87Tm0nz.png)
...
![Table showing calc.exe with associated DLLs: CRYPTBASE.DLL, edputil.dll, MLANG.dll, PROPSYS.dll, Secur32.dll, SSPICLI.DLL, WININET.dll, and their functions.](KJ5drQvVSHvl.png)
...
![Command prompt running calc.exe, desktop showing WININET.dll and calc icons, with a popup message 'Hello from DllMain!' indicating Reflective DLL Injection.](duhSJ8e8XLLI.png)
...
![Filter Current Log window with options for event level, event logs set to Microsoft-Windows-Sysmon/Operational, and Event ID 7.](rLEr5Sqv4pRJ.png)

Subsequently, we search for instances of "calc.exe", by clicking "Find...", to identify the DLL load associated with our hijack.

![Sysmon log entry: Image loaded, ProcessID 6212, Image calc.exe, ImageLoaded WININET.dll, Signed false, User DESKTOP-N33HELB\Waldo. Find dialog open for 'calc.exe'.](LtTjj3ootUus.png)
...
![Sysmon log entry: Image loaded, ProcessID 5464, Image calc.exe, ImageLoaded wininet.dll, Signed true, User DESKTOP-N33HELB\Waldo. Find dialog open for 'calc.exe'.](fuw8UKTW0tRc.png)
...
![Task Manager showing processes like Microsoft.Photos.exe, msedge.exe, powershell.exe, ProcessHacker.exe, with CPU and memory usage details.](tAqnmWpyUHxf.png)
...
![Task Manager tooltip for powershell.exe, showing file path, version 10.0.19041.546, signed by Microsoft, console host conhost.exe (5092).](XsUlGaAunoi9.png)
...
![Image showing clr.dll and drjit.dll with memory addresses, sizes, and descriptions for Microsoft .NET Runtime components.](3NYOHdC46rYr.png)
...
![Tooltip for spoolsv.exe showing file path, version 10.0.19041.1288, signed by Microsoft, and associated with Print Spooler service.](LcYCnNXkQgoP.png)

After the injection, we observe that "spoolsv.exe" transitions from an unmanaged to a managed state.

![Tooltip for spoolsv.exe showing file path, version 10.0.19041.1288, signed by Microsoft, associated with Print Spooler service, and managed by .NET.](Hj2GQOEiiJ1F.png)
...
![Properties window for spoolsv.exe showing modules like advapi32.dll, amsi.dll, APMon.dll, with base addresses, sizes, and descriptions.](NpmMfILzW2ad.png)![Sysmon Event 7: Image loaded, ProcessID 2792, Image spoolsv.exe, ImageLoaded clr.dll, Microsoft .NET Runtime, signed by Microsoft, User NT AUTHORITY\SYSTEM.](5s0x15QvIAGX.png)
...
![Event ID 10: ProcessAccess details process access events for detecting hacking tools targeting processes like Lsass.exe for credential theft.](nL6uKbUzbW1t.png)![Sysmon Event 10: Process accessed, SourceImage AgentEXE.exe, TargetImage lsass.exe, SourceUser DESKTOP-R4PEEIF\waldo, TargetUser NT AUTHORITY\SYSTEM.](ynlCGi4U3Zlu.png)
...
![Diagram of Event Tracing for Windows (ETW) showing data flow from Providers A, B, and C to Sessions in ETW, controlled by a Controller, with events logged to Trace Files and delivered to a Consumer.](bHYgRlukJsZf.png)
...
![Windows desktop showing Command Prompt with performance tracking context details and Performance Monitor app search result.](Mjw5XNy1Au7A.png)

![Performance Monitor window displaying a list of running Event Trace Sessions.](iDYxUOXNWQgl.png)

2. ETW Provider metadata can also be viewed through the [EtwExplorer project](https://github.com/zodiacon/EtwExplorer).

![ETW Explorer window showing search results for 'PowerShell' with two providers listed, including GUIDs.](azJv5XbJFV9t.png)
...
![Process Hacker window displaying a list of running processes with details like PID, CPU, and memory usage.](9zjZO2fjzb4O.png)
...
![Process list showing spoolsv.exe with PID 2792 and conhost.exe with PID 648.](080TErlu6V8U.png)
...
![Desktop showing Process Hacker with running processes, Event Viewer with Sysmon logs, and PowerShell executing a command.](md2zuI7NPLBz.png)
...
![Desktop showing Process Hacker with running processes, PowerShell executing commands, and Command Prompt running SilkETW for event tracing.](FUICqhqnlqNI.png)
...
![Desktop showing Process Hacker with running processes, PowerShell executing commands, and Notepad displaying process details with a search for PID 2508.](zwqUUPLRci0g.png)
...
![Sysmon Event 7 details showing image load information for Seatbelt.exe, including process ID, file path, and signature status.](MvhZ5oudXNK5.png)![Sysmon Event 7 showing image load details for Seatbelt.exe, including process ID, file path, and signature status as valid.](xP3ShuQQNHhw.png)
...
![Desktop showing Windows Explorer with Temp folder, Command Prompt running SilkETW, PowerShell executing Seatbelt.exe, and Notepad displaying JSON data.](zWNcngD4hs4D.png)
...
![Sysmon Event 1 showing process creation details for SecurityHealthHost.exe, including process ID, file path, and command line.](HI2Q2tFUWehg.png)
...
```
   ...
   - `| Where-Object {$_.Properties[21].Value -like "*-enc*"}`: This portion of the command further filters the retrieved events. The '\|' character (pipe operator) passes the output of the previous command (i.e., the filtered events) to the 'Where-Object' cmdlet. The 'Where-Object' cmdlet filters the output based on the script block that follows it.
     ...
     - `.Properties[21].Value`: The `Properties` property of a "Process Create" Sysmon event is an array containing various data about the event. The specific index `21` corresponds to the `ParentCommandLine` property of the event, which holds the exact command line used to start the process.
       ![Sysmon Event 1 showing process creation details for mmc.exe, including process ID, file path, and command line, with high integrity level.](k1y7CYWMJSAl.png)
