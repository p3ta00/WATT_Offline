...
![Cycle diagram showing four steps: 1. Ingest Traffic, 2. Reduce Noise by Filtering, 3. Analyze and Explore, 4. Detect and Alert.](oHJu085zjMze.png)
...
![Comparison of OSI and TCP/IP models: OSI has 7 layers including Application, Presentation, Session, Transport, Network, Data-Link, and Physical. TCP/IP has 4 layers: Application, Transport, Internet, and Link.](BUYIgBlIEKyo.png)
...
![Comparison of OSI and TCP/IP models: OSI has 7 layers including Application, Presentation, Session, Transport, Network, Data-Link, and Physical. TCP/IP has 4 layers: Application, Transport, Internet, and Link. PDU types are Data, Segment/Datagram, Packet, Frame, and Bit.](woQDnaZmIkj9.png)
...
![Diagram showing PDU types: Data, Segment/Datagram, Packet, Frame, Bit. Network packet details include Ethernet II, IPv4, and UDP headers with source and destination addresses.](ir2fTFDxMyc4.png)
...
![Network interface configuration for en0: flags, MAC address, IPv6 and IPv4 addresses, netmask, and status details.](B5B1XeYiHGDS.png)
...
![Network interface configuration for en0: flags, MAC address, IPv6 and IPv4 addresses, netmask, and status details.](Xp7DZJILPOGR.png)
...
![Network interface configuration for en0: flags, MAC address, IPv6 and IPv4 addresses, netmask, and status details.](u6qDvLvug4Mg.png)
...
![World map showing IPv6 adoption: Darker green indicates higher deployment and fewer connectivity issues; lighter green indicates less deployment.](KXkunGcFq6mx.png)
...
![Network packet capture showing TCP connections between IPs 192.168.1.140 and 174.143.213.184, with protocols TCP and HTTP, displaying sequence and acknowledgment numbers.](XSwGlLxuJfax.png)
...
![Network packet capture showing TCP connections between IPs 192.168.1.140 and 174.143.213.184, with protocols TCP and HTTP, displaying sequence and acknowledgment numbers, including SYN, ACK, and FIN flags.](ZnteUn7bygWH.png)
...
![sparkles-icon-decoration](uRAw4IBe1xBU)
...
![Network packet capture showing TCP and TLSv1.3 connections between IPs 192.168.86.243 and 104.20.55.68, with SYN, ACK, and Application Data details.](Qda9FD9aIoOH.png)
...
![FTP traffic log showing commands and responses between IPs 172.16.146.1 and 172.16.146.2, including directory changes, file transfers, and connection status.](paSxrUxADREY.png)
...
![Network packet capture showing SMB and TCP traffic between IPs 192.168.199.132 and 192.168.199.133, including protocol negotiation and logon failure errors.](7zWthnPS58Qh.png)
...
Enable step-by-step solutions for all questions
![sparkles-icon-decoration](4thvaRsRassK)
...
![Network packet capture showing FTP communication between IPs 172.16.146.2 and 172.16.146.1, including welcome message and password request.](eyVSGJdGkOX7.png)
...
With the basic string in the command line above, we utilize TShark to capture on en0, specified with the `-i` flag and the `-w` option to save the capture to a specified output file.
...
![Network packet capture in Termshark showing TCP and TLSv1.3 traffic between IPs 172.16.146.2 and 140.82.114.5, with details on Ethernet, IP, and TCP protocols.](dLKkdYIpdlF9.png)
...
![Termshark interface showing network packet details with a help overlay listing keyboard shortcuts for navigation and filtering.](seowyKy1YU23.png)
...
![Wireshark capture showing HTTP and TCP traffic between IPs 10.1.1.101 and 10.1.1.1, with details on GET requests and packet data in hex and ASCII.](735VD06NhxwL.png)
...
![Wireshark capture showing HTTP and TCP traffic between IPs 10.1.1.101 and 10.1.1.1, with details on GET requests and packet data in hex and ASCII.](l7t6vUzxVMVg.png)
...
![GIF showcasing the capture options.](tvLKBD3D2Nw1.gif)
...
![Toolbar with menu options and icons for file operations, search, and filter application.](fejv193H6Gc2.jpg)
...
Before we apply a capture filter, let us take a look at the built-in filters. To do so:
Click on the capture radial at the top of the Wireshark window → then select capture filters from the drop-down.

#### Filter List

![Wireshark capture filters list with expressions for filtering by Ethernet, IP, TCP, UDP, and specific ports.](5f6dcjgb5O2V.png)
...
![Wireshark capture options showing interfaces, promiscuous mode enabled, and TCP filter applied.](6Jh8NcC9fFsu.png)
...
select the bookmark in the Toolbar → , then select an option from the drop-down. Alternatively, place the cursor in the text radial → and type in the filter we wish to use. If the field turns green, the filter is correct. `Just like in the image below.`

#### Applying Display Filters

![Wireshark interface showing filtered TCP traffic between IPs 162.159.134.234 and 192.168.86.211, with protocols TLSv1.2 and TCP.](7FKWg0NHweWz.png)
...
![Wireshark interface showing address statistics, protocol hierarchy, and conversation details for network traffic analysis, including IP addresses, packet counts, and protocol breakdowns.](ANsNmwgh3PxW.png)
...
![Wireshark Analyze menu showing options for display filters, applying filters, enabled protocols, and expert information.](LDpEsbNNnnPb.png)
...
![GIF showcasing the 'Follow a Stream' functionality.](S15evjQn5yOm.gif)
...
![Wireshark capture showing TCP and Telnet packets between IPs 10.100.18.5 and 10.100.16.1, with sequence and acknowledgment numbers.](gVUbj1Vq0V0C.png)
...
![GIF showcasing the extraction of files from an HTTP stream.](BA8ptiD6SHXf.gif)
...
![Wireshark capture showing FTP requests and responses between IPs 172.16.146.1 and 172.16.146.2, including anonymous login and directory changes.](4QRyMv1FaiAe.png)
...
![Wireshark capture showing FTP requests between IPs 172.16.146.1 and 172.16.146.2, including anonymous login and directory commands.](ITKaVoeQTYZn.png)
...
![Wireshark capture showing FTP data transfer between IPs 172.16.146.2 and 172.16.146.1, including retrieval of 'secrets.txt' and 'Shield-prototype-plans' files.](Ur9LroWChmsm.png)
...
When examining the traffic, we captured, was any traffic pertaining to FTP noticed?
Who was the server for that traffic?
...
Access to the lab environment to complete the following tasks will require the use of [XfreeRDP](https://manpages.ubuntu.com/manpages/trusty/man1/xfreerdp.1.html) to provide GUI access to the virtual machine so we can utilize Wireshark from within the environment.
...
![Wireshark capture showing ARP and TCP packets between IPs 10.129.43.4 and 10.129.43.29, with conversation statistics below.](iIsaQQ2mtRBQ.png)
...
![Wireshark Protocol Hierarchy Statistics showing percentages and bytes for various protocols, including TCP and UDP.](lBnHA3uUW63K.png)
...
![Wireshark capture showing ARP and NAT-PMP packets between VMware and IPs 10.129.43.4 and 10.129.0.1, including M-SEARCH and TCP requests.](BmEbuKJAB8PI.png)
...
![Wireshark capture showing TCP packets between IPs 10.129.43.4 and 10.129.43.29, filtered by UDP and ARP protocols.](hi9AwqjderAp.png)
...
![Wireshark capture showing TCP packets between IPs 10.129.43.4 and 10.129.43.29, with sequence and acknowledgment details.](hHVDF0DCd4q2.png)
...
![Command prompt showing Windows version, IP configuration, directory listing, and user creation commands.](VWfk7I96aPQ5.png)
...
![Wireshark interface with no packets displayed, showing Remote Desktop Protocol filter applied.](EUW6I83Uq2zj.png)
...
![Wireshark capture showing TCP and TLSv1.2 packets between IPs 10.129.43.27 and 10.129.43.29, filtered by port 3389.](sOX0KdBWh1pX.png)
...
![Wireshark Preferences window showing TLS settings, including RSA key list and options for reassembling TLS records.](yHSuS3Uvh6Tc.png)
...
![Wireshark TLS Decrypt settings showing IP 10.129.43.29, port 3389, protocol tpkt, and key file path.](mG4imxsV7Rc9.png)

When filtering once again on RDP, we should see some traffic in the display.

#### RDP In The Clear

![Wireshark capture showing RDP packets between IPs 10.129.43.27 and 10.129.43.29, detailing various PDU types and actions.](dUgHlU0wKB4d.png)
...
![Wireshark capture showing TCP and TLSv1.2 packets between IPs 10.129.43.27 and 10.129.43.29, filtered by port 3389, with details on ignored records and a cookie value.](KNKFrSOI1tnG.png)
