...
![sparkles-icon-decoration](j1nDzySfRhXl)
...
![image](lmBmnilnQkcx.png)

Now search within the Packet bytes for any `auth-session` cookies as follows.

Navigate to `Edit` -\> `Find Packet`![image](iMqz1otB3X6G.png)

Left-click on `Packet list` and then click `Packet bytes`![image](lyV6FE49xHIq.png)
...
Enable step-by-step solutions for all questions
![sparkles-icon-decoration](7tAxSP9qctT5)
